        ...
        public class NoirRenderer : ImageListView.ImageListViewRenderer
        {
            ...
            public override void DrawPane(Graphics g, ImageListViewItem item, Image image, Rectangle bounds)
            {
                ...
                if (item != null && image != null)
                {
                    ...
                    // File type
                    string fileType = item.GetSubItemText(ColumnType.FilePath);
                    if (ImageListView.Columns.HasType(ColumnType.FilePath) && ImageListView.Columns[ColumnType.FilePath].Visible && bounds.Height > 0 && !string.IsNullOrEmpty(fileType))
                    {
                        using (Brush bCaption = new SolidBrush(Color.FromArgb(196, 196, 196)))
                        using (Brush bText = new SolidBrush(Color.White))
                        {
                            int y = Utility.DrawStringPair(g, bounds, ImageListView.Columns[ColumnType.FilePath].Text + ": ",
                ...
                if (ImageListView.View == Manina.Windows.Forms.View.Thumbnails)
                {
                    ...
                    using (StringFormat sf = new StringFormat())
                    {
                        ...
                        using (Brush bItemDetails = new SolidBrush(ImageListView.Colors.PaneLabelColor))
                        {
                            ...
                            string resolution = item.GetSubItemText(ColumnType.MediaInfo);
                            ...
                            string fileSize = item.GetSubItemText(ColumnType.MediaSize);
                ...
                if (ImageListView.View != Manina.Windows.Forms.View.Details)
                {
                    ...
                    if ((state & ItemState.Hovered) != ItemState.None)
                    {
                        ...
                        using (StringFormat sf = new StringFormat())
                        {
                            ...
                            string fileSize = item.GetSubItemText(ColumnType.MediaSize);
                            if (!string.IsNullOrEmpty(fileSize))
                                details += item.GetSubItemText(ColumnType.MediaSize);
